        ...
        if(el.categories?.length>0){
            setHovering(el.categories.map(elem=>{
                ...
                if(elem.categories?.length>0){
                    arr = elem.categories.map(elema=><div><NavLink onClick={handleClick}  to={`/catalog/${elema.id}`}>{ elema.name }</NavLink></div>)
                }
                return <>
                    <div style={{fontWeight:"bold"}}><NavLink  onClick={handleClick} to={`/catalog/${elem.id}`}>{ elem.name }</NavLink></div>
                    {arr && arr.map(i=>i)}
                </>
            }))
        }
        ...
        <>
            <Popper id={id} open={open} anchorEl={anchorEl}>
                <div className={st.testModal}>
                    <div className={st.firstPlace}>{categories && categories.map ( el=>{ return <div key={el.id} style={ { border : "none",width:"max-content"} }
