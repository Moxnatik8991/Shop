...
const getItemById= async (itemId)=>{
    let result = (await axios.get( `https://5iaf6t.realhost-free.net/api/Product/Get/${ itemId }` )).data.result
    return result
}
const getItemsByCategory= async (catId)=>{
    let testUrl= encodeURIComponent(`[{"id": "categoryId", "value": "${catId}"}]`);
    let result = (await axios.get(`https://5iaf6t.realhost-free.net/api/Product/GetWithFilteringAndPagination?ColumnFilters=${ testUrl }`)).data.result
    return result
}
